...
expand_grid(A = 10^(1:6),
  ...
  mutate(embedding_cost = (A+B) * tokens_per_string * embedding_cost_per_token,
         completion_cost = A * 5 * tokens_per_completion * completion_cost_per_token) |> 
...
Note that cost scales more quickly with the size of `dfA` than with `dfB`, because the LLM prompts for validation are more costly than retrieving embeddings. For particularly large datasets, one can significantly reduce costs by blocking and/or increasing the probability threshold during the validation step.
