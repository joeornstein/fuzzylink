...
The goal of the `fuzzylink` package is to allow users to merge datasets with non-exact matches on key identifying variables. Suppose, for example, you have the following two datasets:
...
First, the function encodes each unique string in `dfA` and `dfB` as a 3,072-dimensional vector called an *embedding*. You can learn more about embeddings [here](https://platform.openai.com/docs/guides/embeddings/embedding-models), but the basic idea is to represent text using a vector of real-valued numbers, such that two vectors close to one another in space have similar meanings.
...
Next, we fit a logistic regression model on the `train` dataset, so that we can map similarity scores onto a probability that two records match. We use both the cosine similarity (`sim`) and a lexical similarity measure (`jw`) as predictors in this model.
...
Finally, we take all name pairs whose match probability is higher than a given threshold and merge them into a single dataset.
...
Because the `fuzzylink()` function makes several calls to the OpenAI API---which charges a [per-token fee](https://openai.com/pricing)---there is a monetary cost associated with each use. Based on the package defaults and API pricing as of February 2024, here is a table of approximate costs for merging datasets of various sizes.

```{r, echo=FALSE}
embedding_cost_per_token <- 0.00013 / 1e3
completion_cost_per_token <- 0.0015 / 1e3
tokens_per_string <- 4.5
tokens_per_completion <- 40
tibble(rows = 10^(1:6)) |> 
  mutate(embedding_cost = rows * tokens_per_string * embedding_cost_per_token,
         completion_cost = (rows / 2) * 5  * tokens_per_completion * completion_cost_per_token) |> 
  mutate(approx_cost = paste0('$', round(embedding_cost + completion_cost, 2)), 
         rows = scales::comma_format()(rows)) |> 
  select(`Total Number of Records` = rows, 
         `Approximate Cost (Default Settings)` = approx_cost) |>  
  knitr::kable()
```

For particularly large datasets, one can significantly reduce costs by blocking and/or increasing the probability threshold during the validation step.
