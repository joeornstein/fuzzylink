...
Next, we compute the *cosine similarity* between each name pair. This is our measure of how closely related two pieces of text are. If you include `blocking.variables` in the call to `fuzzylink()`, the function will only consider *within-block* name pairs (e.g. it will only compute similarity scores for records with an exact match on last name). I highly recommend blocking wherever possible, because it reduces cost and significantly speeds up computation.
