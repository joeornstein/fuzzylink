...
check_match <- function(string1, string2,
  ...
  if(batch_size == 1){
    ...
    while(start_index <= length(string1)){
      ...
      p[[3]] <- list(role = 'user',
                     content = 'For each pair of names, decide whether they probably refer to the same entity. Nicknames, acronyms, abbreviations, and misspellings are all acceptable matches. Respond with a list of only "Yes" or "No".')
